#+TITLE: Sunset guitar
#+AUTHOR: Paul Provost
#+EMAIL: paul@bouzou.org
#+DESCRIPTION: A guitar for my 50th birthday
#+FILETAGS: @sunset:@musicmaker
#+SETUPFILE: ../.publish-style

* [[file:sunset/][Assets]]
** Comparison shots
*** Body comparisons
    - [[file:sunset/Combined.pxm][Merged]]
    - [[file:sunset/k2bbjk.jpg][Sadowsky Devin Townsend custom]]
    - [[file:sunset/ml3rcr.jpg][Chapman ML3]]
    - [[file:sunset/Fender_Telecaster_American_Vintage_1952_transparent.png][Fender Telecaster]]
*** Simulation 1 vs Simulation 2
    
** Propositions
   - [[file:sunset/Simulation_1.jpg][file:sunset/Simulation_1_thumb.jpg]]
   - Simulation 2
     - [[file:sunset/Simulation_2.1.jpg][file:sunset/Simulation_2.1_thumb.jpg]]
     - [[file:sunset/Simulation_2.2.jpg][file:sunset/Simulation_2.2_thumb.jpg]]
   - Simulation 3
   [[file:sunset/Simulation_3.jpg][  file:sunset/Simulation_3_thumb.jpg]]
   - Simulation 4
     [[file:sunset/Simulation_4.jpg][file:sunset/Simulation_4_thumb.jpg]]

** Reference
   Oak Grigsby 5-way 4-pole switch [[file:sunset/swl54connections.jpg][connections]]
** Wiring
   - [[file:sunset/wdu_hsh5l11_02.jpg][Original HSH wiring]] from [[http://www.guitarelectronics.com/product/WDUHSH5L1102/WDU-HSH5L11-02.html][GuitarElectronics.com - WDU-HSH5L11-02]]
   - My mod:
     file:sunset/wdu_hsh5l11_02_modded.png
   - A second mod to decouple the volume pots (as on my Liberatore
     guitar, and as per [[http://www.dimarzio.com/sites/default/files/diagrams/2h2v1t_3w_all.pdf][DiMarzio]]'s wiring diagram), as suggested in
     [[http://www.premierguitar.com/articles/21106-mod-garage-decouple-your-les-pauls-volume-controls][this article]].
     file:sunset/wdu_hsh5l11_02_modded_fixed.png

   - Last mod, to make the tone control only act on the neck pickup,
     and maybe the middle also. Leave the bridge pickup alone when
     it's on by itself, basically. For EBow work.

     file:sunset/wdu_hsh5l11_02_modded_fixed2.png
